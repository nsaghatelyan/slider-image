...
The slider uses auto generated shortcodes for making it easier to add it to the custom location.
...
**Navigation type**
Plugin offers three type of navigation's, which are the following: Dots, Thumbnails and No Navigation.
...
* 2) Added thumbs positioning optons (top,bottom,left,right)
* 3) Added arrows positioning (top,bottom,left,right)
* 4) Added new functionality to slider supporting Drag&Drop on front end 
...
* 3) We have added image arrows option for thumbnails, allowing to have arrows on thumbnails.
...
* 16) Added video Slide URLs to be visible in the admin panel
...
* 2) Added the new featured plugins page to demonstrate new plugins.
...
* 2) Security update: Finalized addition of nonces for edit/create/delete requests. 
...
* Background Color. Select the background color for the image in the slider, when Image Behavior is set on ”Natural” this shows some background in the slider. The color will be visible with during the slide change within specific effects.
...
* Slide has a background, which can be seen if you select Image Behavior - Natural from general Options, this option will show every image with its natural dimensions, and as the images are different the slider can have a background, in case if you want to back the background of the slider transparent then use the following codes:
